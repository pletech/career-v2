...
インフラ > サーバー、合計 **24 カテゴリ / 236 アクション**。

- ✅ **STEP1（運用監視補助）**: 業務カテゴリ 11 種 / 120 アクション
- ✅ **STEP2（運用監視・一次対応）**: 業務カテゴリ 7 種 / 59 アクション（監視・異常検知 / 障害状況の一次確認 / 初動対応の実施 / 復旧確認・完了処理 / エスカレーション・引き継ぎ / インシデント記録・進行管理 / 定常運用の自立実施）
- ✅ **STEP3（運用保守・二次対応）**: 業務カテゴリ 6 種 / 57 アクション（監視分析・改善提案 / 原因調査・切り分け / 復旧対応・恒久対策の検討 / 二次対応の受付・進行管理 / 定期保守・変更作業 / 手順書・ナレッジ整備）

STEP2 の 59 件のうち 11 件、STEP3 の 57 件のうち 7 件は、**引き継いだ業務を支援なしで実施できるかを確認する項目**です（下記）。
...
下の段階は「補助があればできる」でチェックできるため、**下の段階をクリアしていても「ひとりでできる」ことにはなりません**。一方ロールアップは `actionChecks[actionId]` を共有するだけなので、下の段階で「補助あり」としてチェックした項目は上の段階でもチェック済みに見え、自立度の再確認が起きません（社外レビューでのご指摘）。

そこで**包含関係ごとに1件**、「『下位カテゴリ』の行動を、支援なしで実施できる」という確認項目を置いています。

- `actionId` は `ind-{下位categoryId}`。カテゴリの親は1つだけなので一意に決まり、項目が増えてもIDがずれません（**チェック状態の保存キー**なので重要）
- 判定は `CraftView.tsx` の `selfCheckActionId` と、そのカテゴリの `includes` の突き合わせ
- 画面では**「この段階で追加」とは別グループ**として表示します（新しい業務ではなく既存業務の再確認なので）。展開したロールアップの中でも同じ扱いです
