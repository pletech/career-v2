...
インフラ > サーバー、合計 **24 カテゴリ / 214 アクション**。

- ✅ **STEP1（運用監視補助）**: 業務カテゴリ 11 種 / 109 アクション
- ✅ **STEP2（運用監視・一次対応）**: 業務カテゴリ 7 種 / 56 アクション（監視・異常検知 / 障害状況の一次確認 / 初動対応の実施 / 復旧確認・完了処理 / エスカレーション・引き継ぎ / インシデント記録・進行管理 / 定常運用の自立実施）
- ✅ **STEP3（運用保守・二次対応）**: 業務カテゴリ 6 種 / 49 アクション（監視分析・改善提案 / 原因調査・切り分け / 復旧対応・恒久対策の検討 / 二次対応の受付・進行管理 / 定期保守・変更作業 / 手順書・ナレッジ整備）
...
全項目を「一人称（ひとりで）」で求めると未経験のメンバーにはチェックが長期間つかず目安として機能しないため、段階ごとに自立度の目安を表示しています。`CraftView.tsx` の `STAGE_AUTONOMY`。

| STEP | 目安 |
|---|---|
| 1 | 補助・確認してくれる人がいればできる |
| 2 | 手順書があれば、原則ひとりでできる |
| 3 | ひとりでできる ＋ 改善や対策を提案できる |

項目ごとにレベル軸（補助あり/一人称/改善提案の3段階）を持たせる案は検討中で、**未導入**です。導入する場合はチェックの保存キーを「段階:アクション」に分ける必要があり、既存の保存データの意味が変わります。
