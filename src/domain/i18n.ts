...
export const STRINGS: Record<Lang, UiStrings> = {
  ja: {
    ...
    langLabel: '表示言語',
    roadmapLegend:
      '縦 = 段階（下が STEP1、上へ登るほど高い段階）／ 横 = 業務の種類。同じ業務は下の段から矢印でつながり、段階が上がるごとに次の業務へ成長していきます。',
    noLine: '（ラインなし）',
  },
  ko: {
    disclaimer:
    ...
    langLabel: '표시 언어',
    roadmapLegend:
      '세로 = 단계(아래가 STEP1, 위로 올라갈수록 높은 단계) / 가로 = 업무의 종류. 같은 업무는 아래 단에서 화살표로 이어지며, 단계가 오를 때마다 다음 업무로 성장합니다.',
    noLine: '(라인 없음)',
  },
};
