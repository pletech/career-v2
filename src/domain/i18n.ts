...
export const STRINGS: Record<Lang, UiStrings> = {
  ja: {
    ...
    langLabel: '表示言語',
    roadmapLegend:
      '縦 = 段階（下が STEP1）／ 横 = 業務の区分（タグ）。タグを開くと「できると言える」素材のチェックリストが並びます。上の段階では下で身につけた素材が引き継がれ、新たに必要な素材（NEW）だけが増えます。',
    noLine: '（ラインなし）',
  },
  ko: {
    disclaimer:
    ...
    langLabel: '표시 언어',
    roadmapLegend:
      '세로 = 단계(아래가 STEP1) / 가로 = 업무의 구분(태그). 태그를 열면 "가능하다고 말할 수 있는" 소재 체크리스트가 나열됩니다. 위 단계에서는 아래에서 익힌 소재가 인계되고, 새로 필요한 소재(NEW)만 늘어납니다.',
    noLine: '(라인 없음)',
  },
};
