...
export const STRINGS: Record<Lang, UiStrings> = {
  ja: {
    ...
    langLabel: '表示言語',
    roadmapLegend:
      '段階の見出しを押すと、その段階の業務カテゴリが開きます（他の段階は畳んで件数のみ）。カテゴリは「何項目中いくつできるか」を示し、8割達成で「達成」になります。上位段階のカテゴリは、下位段階のカテゴリを1行に畳んで取り込み（押すとその段階へ）、さらにその段階で新たに必要な項目を持ちます。',
    noLine: '（ラインなし）',
  },
  ko: {
    disclaimer:
    ...
    langLabel: '표시 언어',
    roadmapLegend:
      '단계 제목을 누르면 그 단계의 업무 카테고리가 열립니다(다른 단계는 접혀 개수만). 카테고리는 "몇 항목 중 몇 개 가능"을 나타내고, 8할 달성 시 "달성"이 됩니다. 상위 단계 카테고리는 하위 단계 카테고리를 1줄로 접어 포함하고(누르면 그 단계로), 그 단계에서 새로 필요한 항목을 더 가집니다.',
    noLine: '(라인 없음)',
  },
};
