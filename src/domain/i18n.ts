...
export const STRINGS: Record<Lang, UiStrings> = {
  ja: {
    ...
    langLabel: '表示言語',
    roadmapLegend:
      '縦 = 段階（下が STEP1）／ 横 = 業務の区分（タグ）。段階の見出しを押すと、その段階のチェックリストが表の中で開きます（他の段階は件数のみ）。上の段階では下で身につけた項目が引き継がれ、新たに必要な項目（NEW）だけが増えます。',
    noLine: '（ラインなし）',
  },
  ko: {
    disclaimer:
    ...
    langLabel: '표시 언어',
    roadmapLegend:
      '세로 = 단계(아래가 STEP1) / 가로 = 업무의 구분(태그). 단계 제목을 누르면 그 단계의 체크리스트가 표 안에서 열립니다(다른 단계는 개수만). 위 단계에서는 아래에서 익힌 항목이 인계되고, 새로 필요한 항목(NEW)만 늘어납니다.',
    noLine: '(라인 없음)',
  },
};
